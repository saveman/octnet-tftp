...
class read_connection : public std::enable_shared_from_this<read_connection>
{
public:
...
private:
    ...
    void send_prepared_packet()
    {
        m_connection_socket.async_send_to(asio::const_buffer(m_out_packet_data.data(), m_out_packet_data.size()),
            m_client_endpoint,
            std::bind(
                &read_connection::on_packet_sent, shared_from_this(), std::placeholders::_1, std::placeholders::_2));
    }
    ...
    void send_next_data_packet()
    {
        ...
        packet.m_data.resize(DEFAULT_BLOCK_SIZE);

        if (!m_reader->read(packet.m_data.data(), DEFAULT_BLOCK_SIZE, bytes_read))
        {
    ...
    void request_next_receive()
    {
        m_connection_socket.async_receive_from(asio::buffer(m_in_packet_data), m_received_packet_endpoint,
            std::bind(&read_connection::on_packet_received, shared_from_this(), std::placeholders::_1,
                std::placeholders::_2));
    }
    ...
    void on_packet_sent(const asio::error_code& ec, std::size_t bytes_transferred)
    {
        ...
        m_send_timeout_timer.async_wait(
            std::bind(&read_connection::on_send_timeout, shared_from_this(), std::placeholders::_1));
    ...
    void terminate()
    {
        m_handler.connection_terminated(shared_from_this());
    }
    ...
    std::array<std::uint8_t, DEFAULT_BLOCK_SIZE + 2> m_in_packet_data;
};
